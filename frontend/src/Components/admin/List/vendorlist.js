...
const VendorList = () => {
    ...
    const filteredVendors = allRoles.filter((item) => {
        const matchesSearchQuery = Object.values(item).some((val) => {
            // Ensure val is not null or undefined and convert to string before checking
            return val !== null && val !== undefined && val.toString().toLowerCase().includes(searchQuery.toLowerCase());
        });

        const matchesCategory = selectedCategory === "All Vendor" ||
            (Array.isArray(item.categoryID) && item.categoryID.some(cat => cat.name === selectedCategory));
