...
const VendorDetails = () => {
    ...
    useEffect(() => {
        const getUserProfileById = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL_ADM}/getuserbyid/${id}`);
                const serRes = await response.json();

                if (response.ok) {
                    setVendorDetails(serRes.user);
                    setTotalProducts(serRes.totalProducts);
                    setIsBlocked(serRes.user.isBlocked);
                } else {
        ...
        if (isConfirmed) {
            try {
                ...
                const response = await axios.patch(
                    `${process.env.REACT_APP_API_URL_ADM}/isblocked/${id}`,
                ...
                console.log('User block status updated successfully:', response.data);
            } catch (error) {
    ...
    return (
        <div
            className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default
            ...
            <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
                <div className="relative z-30 mx-auto -mt-8 sm:-mt-22 h-16 w-full max-w-16 rounded-full p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
                    <div className="relative drop-shadow-2">
                        {vendordetails && vendordetails.photo ? (
                            <img
                            ...
                            <div className="bg-green-300 w-16 h-16 sm:w-full sm:h-28 md:h-36  flex items-center justify-center bg-gray-300 rounded-full text-lg font-semibold text-white">
                                {`${vendordetails && vendordetails.username.charAt(0) || ''}${vendordetails && vendordetails.username.split(' ')[1]?.charAt(0) || ''}`}
                            </div>
                        )}
                    </div>
                </div>
                <div className="mt-4">
                    ...
                    <div className="flex gap-5 justify-center">
                        ...
                        {vendordetails.categoryID.map((res) => {
                            return (
                    ...
                    <div className="mx-auto mt-4.5 mb-5.5 grid max-w-94 grid-cols-3 rounded-md border border-stroke py-2.5 shadow-1 dark:border-strokedark dark:bg-[#37404F] cursor-pointer">
                        ...
                        <div className="flex flex-col items-center justify-center gap-1 px-4 xsm:flex-row">
                            <span title="click Unapproved" className={`p-1.5 text-xs font-medium uppercase tracking-wider ${isBlocked ? 'text-red-800 bg-red-200 dark:text-red-200 dark:bg-red-600' : 'text-green-800 bg-green-200 dark:text-green-200 dark:bg-green-600'} rounded-lg`}
                        ...
                        <div className="flex flex-col items-center justify-center gap-1 px-4 xsm:flex-row">
                            <span className="font-semibold text-black dark:text-white">
                                <FaPhone className="text-black dark:text-white" /> {/* Phone icon */}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="mx-auto max-w-180">
                        ...
                        <p className="mt-4.5">
                            {vendordetails.addressID
                                ? `${vendordetails.addressID.address_line1}, ${vendordetails.addressID.address_line2}, ${vendordetails.addressID.city}, ${vendordetails.addressID.state}, ${vendordetails.addressID.postal_code}, ${vendordetails.addressID.country}`
