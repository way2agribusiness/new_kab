...
const Products = ({ user, isproPage, searchState, searchCity, category }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [currentPage, setCurrentPage] = useState(1);
    const [products, setProducts] = useState([]);
    const [totalPages, setTotalPages] = useState(1);
    const [itemsPerPage] = useState(isproPage && isproPage ? 8 : 12); // Number of products per page
    ...
    }, [searchState, searchCity, category, currentPage, searchTerm]);
    ...
    return (
        <>
            {!isproPage && (
                <Helmet>
                    <title>All Agricultural Products Deals at Lower Prices</title>
            ...
            <div>
                ...
                {/* Pagination section */}
                <div className={`flex items-center justify-center mt-4 ${isproPage && isproPage ? 'hidden' : 'block'}`}>
                    <button
