...
const MyFPOList = ({ user, setSelectedComponent }) => {
    const [fpoData, setFpoData] = useState([]);
    const [isCopied, setIsCopied] = useState(false);


    const getAllFPOs = async (id) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_FPO}/data/getbyseller/${id}`);
    ...
    const handleClick = (endpoint) => {
        const url = `${process.env.REACT_APP_API_FPO_BY_NAME}/${endpoint}`;

        // Copy the URL to the clipboard
        navigator.clipboard.writeText(url).then(() => {
            // Update the state to reflect that the link has been copied
            setIsCopied(true);
    ...
    return (
        <div className="flex flex-col mb-5">
            <h4 className="text-gray-800 text-center font-semibold">MY FPO LIST</h4>
                    ...
                    <div className="flex justify-between mt-2">
                        <div className="">
                            <div className="flex gap-5">
                                <div>
                                    <button
                                        className={`text-sm ${isCopied ? 'bg-success' : 'bg-slate-500'} hover:bg-slate-400 text-white p-2 rounded-md`}
                                        onClick={handleClick.bind(this, item.slug)}
                                    >
                                ...
                                <div className="">
                                    <WhatsappShareButton
                                        url={`${process.env.REACT_APP_API_FPO_BY_NAME}/${item.slug}`}
                                    >
                                        <button className="flex bg-slate-500 hover:bg-slate-400 p-1.5 text-white rounded-md">
                                            <FaWhatsapp className="me-2 mt-1 dark:text-success" />
                                            <span>Share</span>
                                        </button>
                                    </WhatsappShareButton>
                                </div>
                            ...
                            <p className="text-orange-500 text-xs">
                                <span className="text-sm font-semibold text-black dark:text-gray">Note : </span>
                                Copy this Fpo Link & share with customer
                            </p>
                        </div>
